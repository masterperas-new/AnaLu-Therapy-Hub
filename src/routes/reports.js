  ...
  if (user.role === 'admin') {
    ...
    const sql = `
      SELECT
        ...
        SUM(CASE WHEN wire_received = 0 AND appointment_date::DATE <= CURRENT_DATE THEN 1 ELSE 0 END) AS owed_appointments,
        COALESCE(SUM(fee_cents), 0) AS total_cents,
        COALESCE(SUM(CASE WHEN wire_received = 1 THEN fee_cents ELSE 0 END), 0) AS paid_cents,
        COALESCE(SUM(CASE WHEN wire_received = 0 AND appointment_date::DATE <= CURRENT_DATE THEN fee_cents ELSE 0 END), 0) AS owed_cents
      FROM appointments
...
router.get('/future', async (req, res) => {
  ...
  try {
    ...
    const sql = `
      ...
      FROM appointments
      WHERE appointment_date::DATE > CURRENT_DATE ${userFilter}
    `;
